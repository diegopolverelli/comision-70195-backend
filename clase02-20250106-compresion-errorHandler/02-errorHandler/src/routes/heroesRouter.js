...
import { altaHeroeError } from '../utils/erroresHeroes.js';
...
router.post('/',(req,res)=>{
    let {name}=req.body
    if(!name){
        CustomError.generarError("Error Alta Producto", "Complete al menos el name", altaHeroeError(req.body), TIPOS_ERROR.ARGUMENTOS_INVALIDOS)
